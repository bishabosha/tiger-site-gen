...
object Dictionary:
  ...
  given [D <: Dictionary]: scalanotation.Reader[D] = UniversalDictionaryReader.asInstanceOf[scalanotation.Reader[D]]

  val UniversalDictionaryReader: scalanotation.Reader[Dictionary] = summon[scalanotation.Reader[scalanotation.Expr]].mapResult({
    case m: e.NamedTupleExpr =>
      def exprAsString(innerpath: String)(x: scalanotation.Expr): Result[String, DecodeError] = result:
        x match
          ...
          case e.NamedTupleExpr(_) =>
            raise(DecodeError.Custom(s"at path $innerpath, expected scalar, got ${x.render}"))
          case e.VectorExpr(_) =>
            raise(DecodeError.Custom(s"at path $innerpath, expected scalar, got ${x.render}"))
      result:
          ...
          case (name = k, value = vs) =>
            if k.startsWith("is") then
              k -> (vs.match{
                ...
                case _ => raise(DecodeError.Custom(s" at path .$k, expected boolean, got $vs"))
              }: Boolean)
                ...
                case vs1: e.VectorExpr =>
                  vs1.elements.zipWithIndex.toList.map({
                    case (vss1: e.VectorExpr, index) =>
                      vss1.elements.zipWithIndex.toList.map({case (e, index2) => exprAsString(s".$k[$index][$index2]")(e).ok})
                    case (_, index) => raise(DecodeError.Custom(s" at path .$k[$index], expected vector, got ${vs.render}"))
                  })
                case _ => raise(DecodeError.Custom(s" at path .$k, expected vector, got ${vs.render}"))
              }: List[List[String]])
            else if k.endsWith("s") then
              k -> (vs.match {
                case vs1: e.VectorExpr =>
                  vs1.elements.zipWithIndex.toList.map({case (e, index) => exprAsString(s".$k[$index]")(e).ok})
                case _ => raise(DecodeError.Custom(s" at path .$k, expected vector, got ${vs.render}"))
              }: List[String])
            else
              k -> exprAsString(s".$k")(vs).ok
        }.toMap)
    case data => Result.Err(DecodeError.Custom(s" Invalid front matter ${data.render}"))
  })
