// Main
//> using scala "3.8.3"
//> using options -experimental -preview -Wsafe-init
//> using dependency "com.lihaoyi::os-lib::0.11.8"
//> using dependency "com.lihaoyi::scalatags::0.13.1"
//> using dependency "com.lihaoyi::os-lib-watch::0.11.8"
//> using dependency "com.lihaoyi::sourcecode::0.4.4"
//> using dependency "com.vladsch.flexmark:flexmark-all:0.64.8"
//> using dependency "org.virtuslab::scala-yaml:0.3.2"
//> using dependency "com.lihaoyi::upickle:4.4.3"
//> using dependency "io.github.bishabosha::scala-object-notation::0.4.1"
