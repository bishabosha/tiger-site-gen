...
So we have all the pieces, are we done? The current complexity is at least O(n^3^). It turns out that we are inefficient in two ways, representation of the graph, and not factoring out redundant computation. Let's optimise.
